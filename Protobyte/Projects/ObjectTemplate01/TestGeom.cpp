...
#define STRIDE ijg::TestGeom::STRIDE


using namespace ijg;

GLuint TestGeom::textureID = 0;
...
TestGeom::TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size, const ProtoColor4f col4) :
ProtoShape3(pos, rot, size, col4), diffuseMapImage("white_tile.jpg") {
	diffuseTextureImageURLs.push_back(diffuseMapImage);
}

TestGeom::TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size, const std::vector< ProtoColor4f > col4s) :
ProtoShape3(pos, rot, size, col4s), diffuseMapImage("white_tile.jpg") {
	diffuseTextureImageURLs.push_back(diffuseMapImage);
}


TestGeom::TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size, const ProtoColor4f col4, const std::string& diffuseMapImage, const Vec2f& textureScale) :
ProtoShape3(pos, rot, size, col4), diffuseMapImage(diffuseMapImage), textureScale(textureScale) {
	diffuseTextureImageURLs.push_back(diffuseMapImage);
}

TestGeom::TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size,
...
// multi-texturing
TestGeom::TestGeom(const Dim3f& size, const Col4f& col4, const std::vector<std::string>& diffuseTextureImageURLs, const Vec2f& textureScale) :
ProtoShape3(Vec3f(), Vec3f(), size, col4), diffuseTextureImageURLs(diffuseTextureImageURLs), textureScale(textureScale){
}
TestGeom::TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f& size, const Col4f& col4, const std::vector<std::string>& diffuseTextureImageURLs, const Vec2f& textureScale) :
...
void TestGeom::init() {
	...
	println(diffuseMapTexture);
}

void TestGeom::clearVectors(){
...
void TestGeom::setShader(const ProtoShader* shader) {
	this->shader = shader;
}
...
void TestGeom::setDiffuseMap(const std::string& diffuseMapImage){
	createDiffuseMapTexture(diffuseMapImage);
}
void TestGeom::setBumpMap(const std::string& bumpMapImage){
	createBumpMapTexture(bumpMapImage);
}
void TestGeom::setReflectionMap(const std::string& reflectionMapImage){
}
void TestGeom::setRefractionMap(const std::string& refractionMapImage){
}
void TestGeom::setSpecularMap(const std::string& specularMapImage){
}

// begin internal texture implementation
void TestGeom::createDiffuseMapTexture(const std::string& diffuseMapImage){// create default texture for diffuseMap
...
void TestGeom::createBumpMapTexture(const std::string& bumpMapImage){
...
void TestGeom::loadBumpMapTexture(const std::string& bumpMapImage){
...
void TestGeom::createReflectionMapTexture(const std::string& reflectionMapImage){
}// loc 2
void TestGeom::createRefractionMapTexture(const std::string& refractionMapImage){
}// loc 3
void TestGeom::createSpecularMapTexture(const std::string& specularMapImage){
}// loc 4


void TestGeom::calcFaces() {
	if (faces.size()>0){
		faces.clear();
	}
...
void TestGeom::textureOn(){
	//glEnable(GL_TEXTURE_2D);
	isTextureEnabled = true;
}

void TestGeom::textureOff(){
	isTextureEnabled = false;

}


void TestGeom::enableNormalMap(float depth){

}

void TestGeom::disableNormalMap(){

}

// for STL output
std::vector<Tup4v> TestGeom::getGeomData(){
	std::vector<Tup4v> vs;
...
void TestGeom::display(RenderMode render, float pointSize) {
...
void TestGeom::move(const Vec3f& v) {
	pos += v;
}

void TestGeom::rotate(const Vec3f& r) {
	rot += r;
}

void TestGeom::scale(const ProtoDimension3f& s) {
	size += s;
}

// transform VBO primitives using glBufferSubData
void TestGeom::transform(const ProtoMatrix4f& mat4){
...
void TestGeom::updateColorBuffer(){
	glBindBuffer(GL_ARRAY_BUFFER, vboID);
...
void TestGeom::updateTextureBuffer(){
