...
#define BUFFER_OFFSET(i) ((void*)(i))
	...
	class TestGeom : public ProtoShape3{
	...
	public:
		...
		TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size,
			const ProtoColor4f col4);

		TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size,
			const std::vector< ProtoColor4f > col4s);

		TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size, const ProtoColor4f col4, const std::string& diffuseTextureImage, const Vec2f& textureScale = Vec2f(1, 1));

		TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f size,
			const std::vector< ProtoColor4f > col4s, const std::string& diffuseTextureImage, const Vec2f& textureScale = Vec2f(1, 1));

		// multi-texturing
		TestGeom(const Dim3f& size, const Col4f& col4, const std::vector<std::string>& textureImageURLs, const Vec2f& textureScale = Vec2f(1, 1));
		TestGeom(const Vec3f& pos, const Vec3f& rot, const Dim3f& size, const Col4f& col4, const std::vector<std::string>& textureImageURLs, const Vec2f& textureScale = Vec2f(1, 1));
	...
	// setters/getters
	inline void TestGeom::setPosition(const Vec3f& pos) {
		this->pos = pos;
		init();
	}

	inline void TestGeom::setRotation(const Vec3f& rot) {
		this->rot = rot;
		init();
	}

	inline void TestGeom::setSize(const ProtoDimension3f size) {
		this->size = size;
		init();
	}

	inline void TestGeom::setColor(const ProtoColor4f col4) {
		this->col4 = col4;
		if (col4s.size() > 0){
	...
	inline Vec3f& TestGeom::getPosition() {
		return pos;
	}

	inline Vec3f& TestGeom::getRotation() {
		return rot;
	}

	inline ProtoDimension3f& TestGeom::getSize() {
		return size;
	}

	inline ProtoColor4f& TestGeom::getColor() {
		return col4;
	}

	inline std::vector<ProtoFace3>& TestGeom::getFaces() {
		return faces;
	}

	inline std::vector<ProtoVertex3>& TestGeom::getVertices() {
		return verts;
	}

	inline  std::vector<float>& TestGeom::getInterleavedPrims(){
		return interleavedPrims;
	}

	inline void TestGeom::setTextureScale(const Vec2f& textureScale) {
		this->textureScale = textureScale;
		updateTextureBuffer();
	}

	inline Vec2f TestGeom::getTextureScale() const {
		return textureScale;
	}

	inline GLuint TestGeom::getVboID() const {
		return vboID;
	}

	inline GLuint TestGeom::getIndexVboID() const {
		return indexVboID;
	}

	inline int TestGeom::getIndicesSize() const{
		return static_cast<int>(inds.size());
	}

	// materials
	inline void TestGeom::setDiffuseMaterial(const Col4f& diffuseMaterial){
	...
	inline void TestGeom::setAmbientMaterial(const Col4f& ambientMaterial){
		materials.ambient = ambientMaterial;

		ambient_loc_U = glGetUniformLocation(ProtoShader::getID_2(), "ambientMaterial");
		//init();
	}
	inline void TestGeom::setSpecularMaterial(const Col4f& specularMaterial){
	...
	inline void TestGeom::setEmissiveMaterial(const Col4f& emissiveMaterial){
	...
	inline void TestGeom::setShininess(float shininess){
		materials.shininess = shininess;
		//init();
	}

	inline const Col4f& TestGeom::getDiffuseMaterial() const{
		return materials.diffuse;
	}
	inline const Col4f& TestGeom::getAmbientMaterial() const{
		return materials.ambient;
	}
	inline const Col4f& TestGeom::getSpecularMaterial() const{
		return materials.specular;
	}
	inline const Col4f& TestGeom::getEmissiveMaterial() const{
		return materials.emissive;
	}
	inline float TestGeom::getShininess(){
		return materials.shininess;
	}

	inline void TestGeom::setMaterials(const Material& materials){
		this->materials = materials;
	}
	inline const TestGeom::Material& TestGeom::getMaterials() const{
		return materials;
	}
