...
namespace ijg {
	...
	std::ostream& operator<<(std::ostream& out, const TestShape& ProtoBlock) {
...
TestShape::TestShape(const Vec3f& pos, const Vec3f& rot, const ProtoDimension3f& size,
	const Col4f& col4) :
	TestGeom(pos, rot, size, col4) {
...
TestShape::TestShape(const Vec3f& pos, const Vec3f& rot, const ProtoDimension3f& size,
	...
	TestGeom(pos, rot, size, col4, textureImageURL, textureScale) {
	for (int i = 0; i < 8; ++i){
		col4s.push_back(col4);
	}
	init();
...
/*!
* Constructor */
TestShape::TestShape(const Vec3f& pos, const Vec3f& rot, const Dim3f& size,
	const std::vector<Col4f>& cols4) :
	TestGeom(pos, rot, size, cols4) {
	init();
}
...
TestShape::TestShape(const Vec3f& pos, const Vec3f& rot, const ProtoDimension3f& size,
	...
	TestGeom(pos, rot, size, cols4, textureImageURL, textureScale) {
	init();
}
